...
Remova de `App.tsx` a função `getPageTitle` e o JSX inteiro da `<aside>`; o shell entra na Task 14. Por enquanto, mantenha a `<aside>` renderizando a partir de `visibleGroups(currentUser.role)` com o mesmo `NavItem`, para o E2E seguir passando. `App.tsx` deve terminar com menos de 250 linhas: estado de sessão, `renderContent()` e a montagem do layout.
...
**Interfaces:**
- Produces: `Button({ variant: 'primary'|'ghost'|'danger', block?, ...})`, `Badge({ status })`, `Card`, `Modal({ open, onClose, title, children })`, `Table` (`Table.Head`, `Table.Row`, `Table.Cell`).
...
export const Button = ({
  ...
  <button
    type={type}
    className={`${BASE} ${VARIANTES[variant]} ${block ? 'w-full' : ''} ${className}`}
    {...props}
  >
    {children}
  </button>
...
- [ ] **Step 8: Atualizar o smoke E2E**

Os rótulos de navegação e os títulos mudaram para caixa de frase. Em `e2e/smoke.spec.ts`, ajuste a tabela `ROTAS` para os rótulos novos de `lib/navigation.ts` (por exemplo `['Vencimento de contratos', 'Vencimento de contratos']`, `['Afastados e licenças', 'Afastados e licenças']`). Como agora rótulo e título vêm da mesma fonte, os dois valores de cada par ficam iguais.
